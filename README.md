...
```
****************************************************************************************************************************************************************************************************************************
Στο συγκεκριμένο σημείο γίνεται η αυτόματη εγγραφή του admin του συστήματος στη βάση δεδομένων. 
```
...
```
Κάνουμε έναν έλεγχο έτσι ώστε αν υπάρχει ήδη κάποιος με ρόλοο admin να μην γίνεται ξανά προσθήκη του admin (Ο έλεγχος αυτός βοηθάει στο να μην προστίθεται ο ίδιος admin κάθε φορά που εκκινούμε την εφαρμογή).
****************************************************************************************************************************************************************************************************************************
...
Ο χρήστης καλείται κατά την αίτηση POST να εισάγει το ticket_id της κράτησης την οποία θέλει να διαγράψει. Γίνεται μια αναζήτηση στο collection reservations με βάση το ticket_id που έδωσε ο χρήστης. Σε περίπτωση που δεν βρεθεί κάποια κράτηση επιστρέφεται αντίστοιχο μήνυμα με κωδικό 404 (Not Found).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/9f26f88a-7fc6-4b4a-9131-a683b5efb830)

Στην αντίθετη περίπτωση που το ticket_id είναι υπαρκτό ψάχνουμε να βρούμε την πτήση στην οποία αντιστοιχεί η συγκεκριμένη κράτηση (Δεν υπάρχει περίπτωση να μην βρεθεί καθώς το flight id που υπάρχει στην κράτηση είναι από προηγούμενο endpoint διασταυρωμένο ότι είναι υπαρκτό).
Άν η κλάση του εισητηρίου που θέλουμε να διαγραφεί είναι economy τότε μέσω των εντολών:
```
updated_ticket_left = int(flight["tickets_left"]) + 1
updated_economy_left = int(flight["economy_left"]) + 1
flights.update_one({"flight_id": flight_id}, {"$set": {"economy_left": str(updated_economy_left), "tickets_left": str(updated_ticket_left)}})
```
ενημερώνουμε τη διαθεσιμότητα των εισητηρίων για τη συγκεκριμένη πτήση, ενώ με την εντολή: 
```
reservations.delete_one(reservation)
```
διαγράφουμε τη συγκεκριμένη κράτηση από το collection reservations.

Τέλος επιστρέφεται μήνυμα επιτυχούς διαγραφής με κωδικό 200 (OK).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/ba8a8a1a-8f97-41ed-9821-c42ac48ad800)

Άν η κλάση του εισητηρίου που θέλουμε να διαγραφεί είναι business τότε μέσω των εντολών:
```
updated_ticket_left = int(flight["tickets_left"]) + 1
updated_business_left = int(flight["business_left"]) + 1
...
Τέλος επιστρέφεται μήνυμα επιτυχούς διαγραφής με κωδικό 200 (OK).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/935128de-4fbd-46a6-b7f4-845ea64372ac)
...
Η μεταβλητή username1 περιέχει το username του εκάστοτε συνδεδεμένου χρήστη.
Με βάση την μεταβλητή αυτή ψάχνουμε στο collection users την εγγραφή του εκάστοτε συνδεδεμένου user και με την εντολή 
```
users.delete_one(existing_user)
```
διαγράφουμε τον συγκεκριμένο χρήστη από τη βάση δεδομένων, ενώ παράλληλα θέτουμε και τη μεταβλητή isloggedin σε False.

Τέλος επιστρέφεται μήνυμα επιτυχούς διαγραφής λογαριασμού με κωδικό 200 (ΟΚ).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/61767005-dcbb-48bf-9d56-5ea6388469e0)
...
O admin καλείται να εισάγει το id της πτήσης της οποίας θέλει να ενημερώσει τις τιμές το οποίο αναζητείται στο collection flights και αν δεν υπάρχει επιστρέφεται μήνυμα λάθους με κωδικό 404 (Not Found).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/83161240-8390-4c7b-b05b-c574e09aa19d)

Επίσης ο admin καλείται να εισάγει νέες τιμές για τα εισητήρια (είτε κατηγοριας economy, είτε κατηγορίας business, είται και για τις δύο κατηγορίες). Αν δεν δώσει κάποιες τιμές για καμία από τις 2 κατηγορίες τότε επιστρέφεται μήνυμα λάθους με κωδικό 400 (Bad Request).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/0fd0048f-1bbd-446e-84f3-564a375b2c8c)

Αλλιώς εάν ανανεώσει κάποια (ή και τις δύο) από τις τιμές τότε με την αντίστοιχη εντολή
```
(μία από τις παρακάτω ανάλογα ποιά τιμή ανανεώθηκε)
...
```
ενημερώνουμε στο collection flights τις τιμές της συγκεκριμένης πτήσης και επιστρέφεται και το αντίστοιχο μήνυμα με κωδικό 200 (ΟΚ).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/729cf138-ce46-47cf-9446-720c27769baa)
...
O admin καλείται να εισάγει το id της πτήσης της οποίας θέλει να δει τις λεπτομέριες, το οποίο αναζητείται στο collection flights και αν δεν υπάρχει επιστρέφεται μήνυμα λάθους με κωδικό 404 (Not Found).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/545ad7be-5dd0-4009-b278-fdf583e01b8d)

Αν το flight_id είναι σωστό, τότε αναζητείται αν υπάρχει κάποιο reservation που να έχει γίνει για τη συγκεκριμένη πτήση. Αν υπάρχει κάποιο reservation τ΄τοτε επιστρέφεται μήνυμα πως δεν μπορεί να ολοκληρωθεί η διαγραφή της πτήσης με κωδικό 400 (Bad Request).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/4f296265-f23e-4d3b-852b-66965f91bc27)

Αν δεν υπάρχουν καταγεγραμμένα reservations για τη συγκεκριμένη πτήση τότε είναι ελεύθερη για να διαγραφεί και έτσι με την εντολή
```
flights.delete_one({"flight_id": flight_id})
```
διαγράφεται η συγκεκριμένη πτήση από το collection flights και επιστρέφεται μήνυμα επιτυχούς διαγραφής με κωδικό 200 (ΟΚ).
****************************************************************************************************************************************************************************************************************************
...
O admin καλείται να εισάγει το id της πτήσης της οποίας θέλει να δει τις λεπτομέριες, το οποίο αναζητείται στο collection flights και αν δεν υπάρχει επιστρέφεται μήνυμα λάθους με κωδικό 404 (Not Found).
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/10526e60-ddb4-4636-ac03-6586cd123dab)

Αν το flight_id είναι σωστό, τότε αναζητούνται όλα τα reservations που έχουν γίνει για τη συγκεκριμένη πτήση και τα προσωπικά στοιχεία των χρηστών που έχουν κάνει κρατήσεις για αυτή τη πτήση αποθηκεύονται σε μια λίστα reservation_data. 

Τέλος στην λίστα result αποθηκεύουμε όλες τις πληροφορίες της συγκεκριμένης πτήσης που θέλουμε να επιστρέψουμε, καθώς και τα reservation_data και τα επιστρέφουμε όλα μαζί σε μορφή JSON.
![image](https://github.com/VaggelisGavrielatos/YpoxreotikiErgasia23_e20019_Gavrielatos_Evangelos/assets/132483480/df0909d2-8b7e-432e-8dd9-ba03ab9df643)

****************************************************************************************************************************************************************************************************************************

# Containerize του Web Service

Το Dockerfile περιγράφει ακριβώς τα βήματα που πρέπει να ακολουθήσει το Docker για να δημιουργήσει το image. Μέσω της εκτέλεσης της εντολής __(sudo) docker-compose up --build__ , το Docker θα εκτελέσει τα παρακάτω βήματα:

__Απόκτηση βάσης εικόνας:__ Το Docker θα ξεκινήσει με μια βάση εικόνας Ubuntu:20.04.

__Ενημέρωση των πακέτων:__ Το Docker θα εκτελέσει την εντολή apt-get update για να ενημερώσει την λίστα των διαθέσιμων πακέτων στο Ubuntu.

__Εγκατάσταση Python 3 και pip:__ Το Docker θα εκτελέσει την εντολή apt-get install -y python3 python3-pip για να εγκαταστήσει την Python 3 και το πακέτο pip3.

__Αναβάθμιση του pip:__ Το Docker θα εκτελέσει την εντολή pip3 install --upgrade pip για να αναβαθμίσει το pip στην τελευταία διαθέσιμη έκδοση.

__Εγκατάσταση Flask και pymongo:__ Το Docker θα εκτελέσει την εντολή pip3 install flask pymongo για να εγκαταστήσει τις απαιτούμενες βιβλιοθήκες Flask και pymongo.

__Δημιουργία καταλόγου /app:__ Το Docker θα εκτελέσει την εντολή mkdir /app για να δημιουργήσει έναν κατάλογο με όνομα "app".

__Δημιουργία καταλόγου /app/data:__ Το Docker θα εκτελέσει την εντολή mkdir -p /app/data για να δημιουργήσει έναν κατάλογο με όνομα "data" μέσα στον κατάλογο "app".

__Αντιγραφή του αρχείου service.py:__ Το Docker θα εκτελέσει την εντολή COPY service.py /app/service.py για να αντιγράψει το αρχείο "service.py" στον κατάλογο "app" μέσα στο εικονικό image.

__Προσθήκη των δεδομένων στον κατάλογο /app/data:__ Το Docker θα εκτελέσει την εντολή ADD data /app/data για να προσθέσει τον κατάλογο "data" (που βρίσκεται στον τρέχοντα κατάλογο) στον κατάλογο "app/data" μέσα στο εικονικό image.

__Ανοιγμα θύρας 5000:__ Το Docker θα εκτελέσει την εντολή EXPOSE 5000 για να ανοίξει τη θύρα 5000 στο εικονικό image, ώστε να είναι προσβάσιμη από τον υπολογιστή του χρήστη.

__Ορισμός του εργασιακού καταλόγου:__ Το Docker θα εκτελέσει την εντολή WORKDIR /app για να ορίσει τον κατάλογο εργασίας στον κατάλογο "app".

__Εκκίνηση της εφαρμογής:__ Το Docker θα εκτελέσει την εντολή python3 -u service.py για να εκκινήσει την Python εφαρμογή "service.py" μέσα στο εικονικό image.
