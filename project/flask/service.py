        ...
        if isuser: #if logged in user has the role of a simple user continue
            ...
            if reservation: #if reservation is found
                ...
                if reservation["ticket_class"] == "economy": #if the ticket is of class economy 
                    ...
                    return Response("You succesfully canceled your economy class ticket reservation!",status=200,mimetype="application/json") #status 200 - OK
                    ...
                    return Response("You succesfully canceled your business class ticket reservation!",status=200,mimetype="application/json") #status 200 - OK                   
            else:      
                return Response('There are no reservations with this ticket ID.',status=404, mimetype='application/json') #status 404 - Not Found
        else:
            return Response("Only users can access this endpoint", status=401, mimetype="application/json") #status 401 - Unauthorized
...
#show all users  (example endpoint for now will delete later) 
...
def create_flight():
    ...
    if isloggedin: #if user is logged in continue
        ...
        if not isuser: #if logged in user has the role of admin continue
            ...
            return Response("Flight succesfully added!",status=200,mimetype="application/json") #status 200 - OK
        ...
        if not isuser: #if logged in user has the role of admin continue        
            ...
            if flight:
                ...
                if not reservation:
                    ...
                    return Response("Flight succesfully deleted!",status=200,mimetype="application/json") #status 200 - OK
                else:
